...
class HomeViewModel extends _$HomeViewModel
    with CancelableWorkerContainer<SourceBundle, void> {
  static HomeViewModel of(dynamic ref) => ref.watch(homeViewModelPod.notifier);

  static HomeViewState watch(dynamic ref) => ref.watch(homeViewModelPod);
  ...
  Future<void> onWork(CancelableTask task, SourceBundle input) async {
    ...
    final mangaSourceList =
        mangaHomeList.map((e) => e.sourceInfo.identify).toList();
    final novelSourceList =
        novelHomeList.map((e) => e.sourceInfo.identify).toList();
    ...
    if (isFirst) {
      currentType = SourceType.values.elementAtOrNull(
              int.tryParse(configMap[configCurrentSourceType] ?? "0") ?? 0) ??
          currentType;
      currentIdentify = configMap[configCurrentSourceIdentify] ?? "";
    }

    final list =
        currentType == SourceType.manga ? mangaSourceList : novelSourceList;
    ...
    if (list.isNotEmpty) {
      ...
      configMap[configCurrentSourceType] =
          SourceType.values.indexOf(currentType).toString();
    ...
    if (errorMsg != null) {
      state = state.copyWith(loading: false, isError: true, errorMsg: errorMsg);
    } else {
      state = state.copyWith(
          loading: false,
          isError: false,
