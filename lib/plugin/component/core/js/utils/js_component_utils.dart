...
class JsComponentUtils {
  ...
  static Future<JsEvalResult> evaluateAsync(
      JavascriptRuntime runtime, String js) async {
  ...
  /// throws ComponentPayload
  static Future<dynamic> jsonDecodeWithCheck(
      JavascriptRuntime runtime, JsEvalResult jsEvalResult) async {
    try {
  ...
  /// need 里的函数名需要定义
  /// noNeed 里的函数名不能定义
  static Future<bool> checkFunction(
      JavascriptRuntime runtime, List<String> need, List<String> noNeed) async {
    final List<String> checkList = [];
    checkList.addAll(need.map((e) => "typeof $e === typeof(Function)"));
    checkList.addAll(noNeed.map((e) => "typeof $e !== typeof(Function)"));

    final result = await runtime.evaluateAsync(checkList.join(" && "));
    return result.stringResult == "true";
  }

  /// 包装 perform 函数
  /// argsNum 为参数数量
  static String getPerformFunctionJsCode(
      String performFuncName, String funcName, int argsNum) {
