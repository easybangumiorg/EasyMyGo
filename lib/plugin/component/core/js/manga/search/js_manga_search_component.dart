...
class JsMangaSearchComponent extends MangaSearchComponent
  ...
  static final _performSearchJSCode = JsComponentUtils.getPerformFunctionJsCode(
      _performMethodSearch, methodNameSearch, 1);
  ...
  Future<String> getInitKey(String key) async {
    try {
      final res = await JsComponentUtils.evaluateAsync(
          _runtime, "$methodNameInitKey($key)");
  ...
  Future<MangaSearchResp> search(String key, String keyword) async {
    final res = await JsComponentUtils.evaluateAsync(
        _runtime, "$_performMethodSearch($key,$keyword)");
    ...
    final respTemp = rt.copyWith(
      data:
          rt.data?.map((e) => e.copyWith(source: sourceInfo.identify)).toList(),
    );
  ...
  Future<bool> isAvailable() async {
    return JsComponentUtils.checkFunction(_runtime,
        [methodNameSearch, methodNameInitKey], [_performMethodSearch]);
  }
